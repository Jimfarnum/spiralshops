...
// Apply comprehensive discounts to order totals
router.post('/apply-discounts', (req, res) => {
...
// Get available discount tiers for a given volume
router.get('/tiers/:annualVolume', (req, res) => {
  const annualVolumeUSD = parseFloat(req.params.annualVolume);
  
  if (isNaN(annualVolumeUSD) || annualVolumeUSD < 0) {
    return res.status(400).json({ error: 'Invalid annual volume' });
  }
  
  const tier = getDiscountTier(annualVolumeUSD);
  
  res.json({
    annualVolumeUSD,
    currentTier: tier,
    estimatedMonthlySavings: (annualVolumeUSD / 12) * (tier.discountBps / 10_000)
  });
});

// Calculate shipping discounts for volume
router.post('/shipping-discount', (req, res) => {
  const { parcelsPerMonth, baseShippingCost } = req.body;
  
  if (!parcelsPerMonth || !baseShippingCost) {
    return res.status(400).json({ error: 'Missing parcelsPerMonth or baseShippingCost' });
  }
  
  const discountPct = logisticsDiscountPct(parcelsPerMonth);
  const discountAmount = baseShippingCost * discountPct;
  const finalCost = baseShippingCost - discountAmount;
  
  res.json({
    baseShippingCost,
    parcelsPerMonth,
    discountPct: Math.round(discountPct * 100), // Convert to percentage
