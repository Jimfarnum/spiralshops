...
router.get("/trips-by-location", async (req, res) => {
  try {
    ...
    const enrichedTrips = filtered.map(trip => ({
      ...
      estimatedValue: trip.cartItems?.reduce((sum, item) => sum + (item.price * (item.quantity || 1)), 0) || 0,
      potentialCustomers: trip.participants.length,
  ...
  try {
    ...
    const stats = {
      ...
      estimatedRevenue: todayTrips.reduce((sum, t) => 
        sum + (t.cartItems?.reduce((itemSum, item) => 
          itemSum + (item.price * (item.quantity || 1)), 0) || 0), 0
      ),
      averageCartValue: todayTrips.length > 0 ? 
        todayTrips.reduce((sum, t) => sum + (t.cartItems?.reduce((itemSum, item) => 
          itemSum + (item.price * (item.quantity || 1)), 0) || 0), 0) / todayTrips.length : 0
    };
